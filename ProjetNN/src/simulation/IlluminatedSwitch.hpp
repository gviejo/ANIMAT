...
class illuminated_switch
{
public:
  // in WORLD coordinate (NOT pixel)
  illuminated_switch(int color, float radius, float x, float y, bool on) 
  {
    _color = color;
    _radius = radius;
    _x = x;
  ...
  void try_to_activate(SPTR<Posture> robot_pos)
  {
  ...
  void link(SPTR<illuminated_switch> o) { _linked_lights.push_back(o); }
...
protected:
  ...
  bool _on;
  bool _activated;
  std::vector<SPTR<illuminated_switch> > _linked_lights;
};

struct ClosestSwitch_f
{
  ClosestSwitch_f(float x, float y) : _x(x), _y(y) {}
  bool operator()(SPTR<illuminated_switch> i1,
		  SPTR<illuminated_switch> i2)
  {
