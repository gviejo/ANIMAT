...
void Display :: _blit_Map()
{
  for (unsigned i = 0; i < _Map->get_pixel_w(); ++i)
    for (unsigned j = 0; j < _Map->get_pixel_h(); ++j)
      if (_Map->get_pixel(i, j) == obstacle)
...
Display :: Display(SPTR<Map> m, SPTR<Robot> r) 
{
  _Map = m;
  _Robot = r;
...
void Display :: _disp_switches()
  {
    for (size_t i = 0; i < _Map->get_illuminated_switches().size(); ++i)
      {
	const illuminated_switch& sw = *_Map->get_illuminated_switches()[i];
...
void Display :: _disp_Lasers()
  {
    for (size_t i = 0; i < _Robot->get_Lasers_size(); ++i)
      {
	unsigned x_Laser = _Map->real_to_pixel(_Robot->get_pos()->x() 
					       + _Robot->get_Laser(i)->get_gap_dist() 
					       * cosf(_Robot->get_pos()->theta() 
						      + _Robot->get_Laser(i)->get_gap_angle()));
	unsigned y_Laser = _Map->real_to_pixel(_Robot->get_pos()->y() 
					       + _Robot->get_Laser(i)->get_gap_dist() 
					       * sinf(_Robot->get_pos()->theta()
						      + _Robot->get_Laser(i)->get_gap_angle()));
	_line(_screen, x_Laser, y_Laser,
	      _Robot->get_Laser(i)->get_x_pixel(),
	      _Robot->get_Laser(i)->get_y_pixel(),
	      0xFF00000);
      }
  }

void Display :: _disp_LightSensors()
  {
    for (size_t i = 0; i < _Robot->get_LightSensors().size(); ++i)
      {
	const LightSensor& ls = _Robot->get_LightSensors()[i];
